...
T_Umgebung=273.15 + 25;                    % Umgebungstemperatur in K 
T_Start_EMaschine = 273.15 + 25;           % Starttemperatur E-Maschine in K
T_Start_Coolant = 273.15 + 25;             % Starttemperatur Kühlmittel in K
% n_EM = 2500;                               % min^-1
% M_EM = 60;                                 % Nm
t_Simulation = 60*60;                      % s
T_Stator_Wicklung_Limit = 273.15 + 120;    % K

n_min = 500;                               % min^-1
n_max = 6000;                              % min^-1
n_Stuetzstellen = 12;                      % -
M_Diskretisierung = 5;                    % Nm
...
% Parameter Zeit
%--------------------------------------------------------------------------
dt_max_Anwender=0.5;                                                       % Durch den Anwender festgelegter maximaler Zeitschritt der Simulation in s -> Je kleiner der Zeitschritt der Simulation ist, desto besser ist die Ergebnisqualitaet der Simulation, desto hoeher ist jedoch der Simulationsaufwand
...
%--------------------------------------------------------------------------
Verlustleistung_Fit = 4; % Fit-Parameter um Temperaturverläufe aus Messung anzunähern, mit ihm wird Gesamtverlustleistung multipliziert
...
eta_EMaschine_Faktor_Volllast=0.97;                                         % Faktor der Volllast fuer das Wirkungsgradkennfeld der E-Maschine
...
radiator_power = 150; % W/K; Kühlleistung des Kreislaufes bzgl. der E-Maschine
...
for c=1:length(n_Sim)
    ...
    M_EM = interp1(M_EMaschine_max(1,:), M_EMaschine_max(2,:), n_Sim(c));
    %test
