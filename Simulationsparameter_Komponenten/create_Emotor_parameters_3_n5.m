...
m_SSL = m_SSL_L + m_SSL_V; % kg; Masse in Statornuten gesamt
    m_WH_L = 1/2*Entwurf.Optionen.Stator_Leitermaterial.rho_Le*Entwurf.Wicklung.l_1w*Entwurf.Wicklung.A_1L*1e-6*Entwurf.Wicklung.z_1n*Entwurf.Wicklung.N_1; % kg; Masse Leiter in Wicklungskopf
    ...
    m_WH_Lack = 1/2*(Entwurf.Wicklung.l_1w*Entwurf.Wicklung.z_1n*Entwurf.Wicklung.N_1)*A_Lack*rho_Lack; % kg; Masse des Isolierlackes im Wickelkopf
m_WH = m_WH_L + m_WH_Lack; % kg; Gesamtmasse des Wickelkopfes
...
cp_WH = (cp_1L*m_WH_L+cp_Lack*m_WH_Lack)/m_WH; % J*(kg*K)^-1

%% Berechnung Gesamtmasse und spezifische Wärmekapazität der gesamten Maschine
m_EM_ges = n_dis*m_HO_a+2*m_HO_p+2*m_EC+n_dis*m_SYO+n_dis*m_STO+n_dis*m_SSL+2*m_WH+n_dis*m_RTO+n_dis*m_RSL+2*m_ER+n_dis*m_RYO+n_dis*m_SH_a+2*m_SH_p; % kg; Gesamtmasse E-Maschine
cp_EM_ges = ((n_dis*m_HO_a+2*m_HO_p)*cp_HO+2*m_EC*cp_EC+n_dis*m_SYO*cp_SYO+n_dis*m_STO*cp_STO+n_dis*m_SSL*cp_SSL+2*m_WH*cp_WH+n_dis*m_RTO*cp_RTO+n_dis*m_RSL*cp_RSL+2*m_ER*cp_ER+n_dis*m_RYO*cp_RYO+(n_dis*m_SH_a+2*m_SH_p)*cp_SH)/m_EM_ges; % J*(kg*K)^-1; mittlere spez. Wärmekapazität E-Maschine
...
R_toEC_p = (0.5*(LF-1)*Entwurf.Geometrie.l*0.5)/lam_HO/(pi/4*((Entwurf.Geometrie.D_1a+2*b_HO)^2-Entwurf.Geometrie.D_1a^2)); % K*W^-1; Wärmeleitwiderstand des Gehäuses axial zu den Gehäuse-Stirndeckeln auf passivem Überhang links und rechts über aktive Länge
    b_WH = (m_WH_L/Entwurf.Optionen.Stator_Leitermaterial.rho_Le+m_WH_Lack/rho_Lack)/(pi*(Entwurf.Geometrie.D_1i+Entwurf.Geometrie.Nut_1.h_1l*1e-3))/(Entwurf.Geometrie.Nut_1.h_1l*1e-3); % m; Hilfsgröße; Breite (axial) des Wickelkopfes
R_toSSL = 0.5*b_WH/lam_WH_axial/(pi/4*((Entwurf.Geometrie.D_1i+2*Entwurf.Geometrie.Nut_1.h_1l*1e-3)^2-Entwurf.Geometrie.D_1i^2)); % K*W^-1; Wärmeleitwiderstand axial im Wickelkopf in beide Richtungen (Nut und Innenluft)
...
R_toIA = (0.5*Entwurf.Geometrie.Nut_1.h_1l*1e-3)/lam_WH_radial/(b_WH*pi*(Entwurf.Geometrie.D_1i+1.5*Entwurf.Geometrie.Nut_1.h_1l*1e-3)); % K*W^-1; Wärmeleitwiderstand Wickelkopf radial zur Außenfläche
R_toIA2 = (0.5*Entwurf.Geometrie.Nut_1.h_1l*1e-3)/lam_WH_radial/(b_WH*pi*(Entwurf.Geometrie.D_1i+0.5*Entwurf.Geometrie.Nut_1.h_1l*1e-3)); % K*W^-1; Wärmeleitwiderstand Wickelkopf radial zur Innenfläche
...
A_ER = A_ER_axial+A_ER_tang_a+A_ER_tang_i; % m^2; Oberfläche des Kurzschlussrings im Kontakt mit Innenluft(auf einer Seite)
    ...
    A_WH_tang_a = A_WH_Faktor*b_WH*pi*(Entwurf.Geometrie.D_1i+2*Entwurf.Geometrie.Nut_1.h_1l*1e-3); % m^2; Wickelkopf Mantelfläche (tangential) außen
    A_WH_tang_i = A_WH_Faktor*b_WH*pi*Entwurf.Geometrie.D_1i; % m^2; Wickelkopf Mantelfläche (tangential) innen
A_WH = A_WH_axial+A_WH_tang_a+A_WH_tang_i; % m^2; Oberfläche des Wickelkopfes im Kontakt mit Innenluft (auf einer Seite)
...
A_IA = 2*(A_WH+A_ER+A_SH+A_EC+A_HO2); % m^2; Gesamtoberfläche Innenluft: benötigt für Aufteilung der Lüfterverluste
